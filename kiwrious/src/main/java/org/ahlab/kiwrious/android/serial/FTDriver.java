...
enum FTDICHIPTYPE {
    FT232RL, FT2232C, FT232H, FT2232D, FT2232HL, FT4232HL, FT230X, CDC, NONE, KIWRIOUS;
}
...
public class FTDriver {
    ...
    private static final UsbId[] IDS = {
            ...
            new UsbId(0x04D8, 0xEC19, 0, 3, FTDICHIPTYPE.KIWRIOUS)  // KIWRIOUS
    };
    ...
    public boolean begin(int baudrate) {
        ...
        Log.w("serialCommunication", "-------------------------  FTDI Begin  ----------------------------");
        Log.w("serialCommunication", "-------------------------------------------------------------------");
        Log.w("serialCommunication", "------------------------- List of Devices -------------------------");
        Log.w("serialCommunication", "-------------------------------------------------------------------");
        for (UsbDevice device : mManager.getDeviceList().values()) {
            Log.w("serialCommunication", "Device : " + device.getProductName());
            Log.w("serialCommunication", "toString : " + device.toString());

            // getPermission(device);
            if (!mManager.hasPermission(device)) {
                Log.w("serialCommunication", "Permission Denied");
                return false;
            } else {
                Log.w("serialCommunication", "---------------------- Permission Granted -------------------------");
                Log.w("serialCommunication", "-------------------------------------------------------------------");
            }
        ...
        if (mSelectedDeviceInfo == null) {
            Log.w("serialCommunication", "mSelectedDeviceInfo == null");
            return false;
        } else {
            Log.w("serialCommunication", "mSelectedDeviceInfo != null");
        }
        ...
        if (mDevice == null) {
            Log.w("serialCommunication", "mDevice == null");
            return false;
        }else {
            Log.w("serialCommunication", "mDevice != null");
        }
        ...
        if (mDevice.getDeviceClass() == UsbConstants.USB_CLASS_COMM) {
            isCDC = true;
            Log.w("serialCommunication", "mDevice.getDeviceClass()  != null");
        } else {
            Log.w("serialCommunication", "mDevice.getDeviceClass()  == null");
            isCDC = false;
        }
        ...
        Log.w("serialCommunication","Device Serial : " + mDeviceConnection.getSerial());
        ...
        if (mSelectedDeviceInfo != null) {
            if (isCDC) {
                ...
                for (int i = 0; i < mSelectedDeviceInfo.mNumOfChannels; ++i) {
                    Log.w("serialCommunication","CLEARING INTERFACE: " + i);
    ...
    public int read(byte[] buf, int channel) {
        ...
        if (isCDC) {
            //           Log.w("serialCommunication", "isCDC");
        ...
        if (buf.length <= mReadbufRemain) {
            if (!mReadPakcetChecker) {
                ...
                //               Log.w("serialCommunication", "mReadPakcetChecker == false");
            } else {
                //               Log.w("serialCommunication", "mReadPakcetChecker == true");
                /*
                ...
                for (int i = 0; i < buf.length; i++) {
                    ...
                    while ((incReadCount - 1) % 10 != (Byte.valueOf(buf[i]) - '0')) {
                        //                      Log.w("serialCommunication", "!!! Lost Data !!! count : "
        ...
        if (mReadbufRemain > 0) {
            ...
            //   Log.w("serialCommunication", "mReadbufRemain > 0");
...
//CHECK        }
        ...
        //     Log.w ("SerialCommunication", "mFTDIEndpointIN[channel]: " + mFTDIEndpointIN[channel] + " mReadbuf: " + mReadbuf + " mReadbuf.length: " + mReadbuf.length);
        //  Log.w ("SerialCommunication", "len " + len + " mPacketSize: " + mPacketSize + " blocks: " + blocks+ " remain: " + remain+ " mReadbufRemain: " + mReadbufRemain);
//
//        for (int j = 0;j < len;j++ ) {
//            Log.w("serialCommunication", "mSerialBuffer: " + mSerialBuffer[j]+ "  mBytesAvailable: " + mBytesAvailable+ "  j: " + j);
//        }
        int rbufindex = 0;
        ...
        if (mReadPakcetChecker) {
            if (ofst > 0) {
                //       Log.w("serialCommunication", "read buf length 2 : " + Integer.toString(ofst));
                totalReadCount += ofst;
                updateReadCount = true;
            }
            if (updateReadCount) {
                //    Log.w("serialCommunication", "Total of Read Count : " + totalReadCount);
                //    Log.w("serialCommunication", "Increment Read Count : " + incReadCount);
...
//            Log.w ("FTDI", "actual_length: " +  actual_length +
//                                "write_buf: " + write_buf +
//                                "write_size: " + write_size);
    ...
    public boolean isConnected() {
        if (mDevice != null && mFTDIEndpointIN != null
            ...
            Log.w("serialCommunication","isConnected == TRUE");
            return true;
        } else {
            Log.w("serialCommunication","isConnected == FALSE");
            return false;
        }
    }
    ...
    public boolean setBaudrate(int baudrate, int channel) {
        Log.w("serialCommunication","^ - ^ - ^ - ^ - ^ - ^");
        Log.w("serialCommunication","setBaudRate");
        if (mDeviceConnection == null) {
            Log.w("serialCommunication","mDeviceConnection == null");
            Log.w("serialCommunication","^ - ^ - ^ - ^ - ^ - ^");
            return false;
        }
        ...
        Log.w("serialCommunication","mSelectedDeviceInfo.mType: " + mSelectedDeviceInfo.mType);
        int index = 0;
        ...
        Log.w("serialCommunication", "Baud rate index : " + index);


        index |= channel; // Ch.A=1, Ch.B=2, ...

        Log.w("serialCommunication", "Baud rate index | channel : " + channel);
        mDeviceConnection.controlTransfer(0x40, 0x03, baud, index, null, 0, 0); // set
        // baudrate

        Log.w("serialCommunication", "Request Type: " + 0x40 + " Request: " + 0x03 + " Baud: " + baud +  " Index: " + index);
        Log.w("serialCommunication","^ - ^ - ^ - ^ - ^ - ^");
        // TODO: check error
        return true;
    }
    ...
    private void initFTDIChip (UsbDeviceConnection conn, int baudrate) {
        ...
        Log.w("serialCommunication","* - * - * - * - * - *");
        Log.w("serialCommunication","initFTDIChip");
        for (int i = 0; i < mSelectedDeviceInfo.mNumOfChannels; ++i) {
            int index = i + 1;
            Log.w("serialCommunication","channel: " + i);
    ...
    private int calcFTDIBaudrate(int baud, FTDICHIPTYPE chiptype) {
        ...
        Log.w("serialCommunication",": - : - : - : - : - :");
        Log.w("serialCommunication","calcFTDIBaudrate");
        int divisor = 0;
        if (chiptype == FTDICHIPTYPE.FT232RL
            ...
            if (baud <= 3000000) {
                ...
                Log.w("serialCommunication", "baud : " + baud + " divisor:  "+ divisor);
            } else {
                Log.w("serialCommunication", "Cannot set baud rate : " + baud
                        + ", because too high.");
                Log.w("serialCommunication", "Set baud rate : 9600");
        ...
        } else if (chiptype == FTDICHIPTYPE.KIWRIOUS) {
            if (baud <= 3000000) {
                divisor = calcFT232bmBaudBaseToDiv(baud, 48000000);
                Log.w("serialCommunication", "baud : " + baud + " divisor:  "+ divisor);
            } else {
    ...
    private boolean setFTDIEndpoints(UsbInterface[] intf, int portNum) {
        ...
        int KiwriousSensor = 1;
        //Kiwrious has 3 interfaces
        // 0 == notification
        // 1 == IN/OUT
        // 2 == Bootloader
        if (KiwriousSensor > 0) {
            Log.w("serialCommunication", "^.^.^.^.^.^.^.^.^.^.^.^.^.^.^.^.^");
            Log.w("serialCommunication", "intf[0] " + intf[0]);
            Log.w("serialCommunication", "^.^.^.^.^.^.^.^.^.^.^.^.^.^.^.^.^");
            Log.w("serialCommunication", "^.^.^.^.^.^.^.^.^.^.^.^.^.^.^.^.^");
            Log.w("serialCommunication", "intf[1] " + intf[1]);
            Log.w("serialCommunication", "^.^.^.^.^.^.^.^.^.^.^.^.^.^.^.^.^");
            Log.w("serialCommunication", "^.^.^.^.^.^.^.^.^.^.^.^.^.^.^.^.^");
            Log.w("serialCommunication", "intf[2] " + intf[2]);
            Log.w("serialCommunication", "^.^.^.^.^.^.^.^.^.^.^.^.^.^.^.^.^");
            if (intf[1] == null) {
                return false;
            }
            Log.w("serialCommunication","Port Number: " + portNum);
            Log.w("serialCommunication", "intf[i] " + intf[1]);
            epIn = intf[1].getEndpoint(1);
            Log.w("serialCommunication", "epIn " + epIn);
            epOut = intf[1].getEndpoint(0);

            Log.w("serialCommunication", "epIn: " + epIn + "  epOut: " + epOut);
            if (epIn != null && epOut != null) {
                mFTDIEndpointIN[1] = epIn;
                mFTDIEndpointOUT[1] = epOut;
                Log.w("serialCommunication", "mFTDIEndpointIN[1] = " + mFTDIEndpointIN[1] );
                Log.w("serialCommunication", "mFTDIEndpointOUT[1] = " + mFTDIEndpointOUT[1] );
            } else {
                return false;
            }
            return true;
    ...
    private boolean setUSBInterface(UsbDevice device, UsbInterface intf,
        ...
        if (device != null && intf != null) {
            ...
            if (connection != null) {
                Log.w("serialCommunication", "open succeeded");
                if (connection.claimInterface(intf, true)) {
                    Log.w("serialCommunication", "claim interface succeeded");
                    ...
                    for (UsbId usbids : IDS) {
                        ...
                        if ((usbids.mVid == 0 && usbids.mPid == 0 && device
                            ...
                            Log.w("serialCommunication", "Vendor ID : " + device.getVendorId());
                            Log.w("serialCommunication", "Product ID : " + device.getProductId());
                            mDevice = device;
                    ...
                    Log.w("serialCommunication","claim interface failed");
                    connection.close();
                }
            } else {
                Log.w("serialCommunication", "open failed");
            }
        }
    ...
    private boolean getUsbInterfaces(UsbDevice device) {
        ...
        for (UsbId usbids : IDS) {
            ...
            if (usbids.mVid == 0 && usbids.mPid == 0
                ...
                Log.w("serialCommunication", "device.getDeviceClass()  " + device.getDeviceClass() + " == " + UsbConstants.USB_CLASS_COMM );
            ...
            if (intf[0] != null) {
                Log.w("serialCommunication", "usbids.mNumOfChannels " + usbids.mNumOfChannels );
                for (int i = 0; i < usbids.mNumOfChannels; ++i) {

                    if (intf[i] != null) {
                        Log.w("serialCommunication", "Found USB interface " + intf[i]);
                        if (!setUSBInterface(device, intf[i], i)) {
                            Log.w("serialCommunication", "SET INTERFACE "+ i+" FAILED!");
                            return false;
                        }
    ...
    private UsbInterface[] findUSBInterfaceByVIDPID(UsbDevice device, int vendorid,
                                                    int productid) {
        Log.w("FTDI Driver", "ext vendorid " + vendorid + " ext productid " + productid);
        Log.w("FTDI Driver", "findUSBInterface " + device);
    ...
    public void getPermission(UsbDevice device) {
        if (device != null && mPermissionIntent != null) {
            Log.w("serialCommunication", "Intent and device exist");
            if (!mManager.hasPermission(device)) {
                Log.w("serialCommunication", "Requesting permission");
                mManager.requestPermission(device, mPermissionIntent);
            }
        } else {
            if (device == null) {
                Log.w(TAG, "Device doesnt exist");

            }
            if (mPermissionIntent == null) {
                Log.w(TAG, "mPermissionIntent doesnt exist");
    ...
    public void usbDetached(Intent intent) {
        ...
        if (mDevice != null && mDevice.equals(deviceName)) {
            if (mDevice.getProductId() != 60441) {
                setUSBInterface(null, null, 0);
                Log.w("serialCommunication", "USB interface removing");
            } else {
                for (int i = 0; i < mSelectedDeviceInfo.mNumOfChannels; ++i) {
                    Log.w("serialCommunication","CLEARING INTERFACE: " + i);
                    setUSBInterface(null, null, i);
